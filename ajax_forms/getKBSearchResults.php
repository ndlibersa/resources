...
if ($nb_packages > 0){
	...
	foreach ($results[0] as $key => $value) {
		echo "<tr ";
		if ($isPaginated) echo "class='invisible'";
		...
		echo '<td class="results_table_cell"> <button class=thickbox onclick=select("package","'.$key.'");>Select</button></td>';
		echo "</tr>";
	}
	echo '</table>';
	echo "</div>";
...
if ($nb_titles > 0){
	...
	echo '<span class="results_type_title"> Issues </span> <a class="moreResults linkStyle';
	if ($isPaginated) echo " invisible";
	echo '" onclick=allResults("'.$_POST['name'].'","'.$_POST['publisher'].'",1);';
	...
	echo '<table class="results_table">';
		foreach ($results[1] as $key => $value) {
			...
			echo '<td class="results_table_cell"> <button class=thickbox onclick=select("title","'.$key.'");>Select</button></td>';
			echo "</tr>";
		}
		echo '</table>';
	...
	if (!$isPaginated) echo "No results, please check your search fields <br/>";
}
