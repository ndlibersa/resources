<?php
//USE ONLY FOR UPGRADING FROM BETA TO VERSION 1.0
...
if ($step == "3"){
	...
	if (count($errorMessage) > 0){
		...
		if ($link->connect_error) {
			$errorMessage[] = "Could not connect to the server '" . $database_host . "'<br />MySQL Error: " . $link->connect_error;
		}else{
...
<title>CORAL Resources Upgrade</title>
...
<?php if(!$step){ ?>
	...
	<h3>Welcome to the CORAL Resources upgrade from Beta to Version 1.0!</h3>
	This installation will:
	<ul>
		<li>Check that you are running PHP 5</li>
		<li>Connect to MySQL and update the CORAL Resources tables</li>
